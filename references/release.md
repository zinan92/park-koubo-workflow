...
2. 读 `references/douyin-cover.md`、其中的默认 preset/两张实际参考图及 `prompts/title-cover.md` 给标题候选；已有选定标题则直接使用。参考上一期封面时只复用风格，本期截图才是人物源。
...
封面从 v2.12 起还需 cover_design、generation_receipt 和四项样式/缩略图检查，完整字段见 [抖音封面协议](douyin-cover.md)。纯平台标题覆盖需明确 cover_title_override；其余情况两张封面与标题逐字联动。
