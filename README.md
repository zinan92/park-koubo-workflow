...
用户需要发布时，Step 14 继续生成标题、使用本期人脸的横竖封面、指定倍速版本和平台草稿。[发布交付说明](references/release.md) 与 [release 检查器](scripts/release_guard.py) 将这些文件绑定到同一版本；改标题后，旧上传记录不能继续算通过。上传草稿不等于允许发布。
